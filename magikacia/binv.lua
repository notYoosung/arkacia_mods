...
local function has_in_gauntlet(itemstack, player, itemname)
    ...
    if invmetastring ~= "" then
        -- minetest.log("invmetastring: " .. invmetastring)
...
function inventorybags.on_use(itemstack, placer, pointed_thing)
    ...
    return inventorybags.on_use_bag(itemstack, placer, pointed_thing)
end
...
local gauntlet_use_secondary = function(itemstack, placer, pointed_thing, bagtable)
    ...
    if placer:is_player() then
        ...
        if controls.players[placer_name].sneak[1] then
            return open_bag(itemstack, placer, bagtable.width, bagtable.height, bagtable.sound_open) --minetest.show_formspec(placer_name, "magic:gauntlet_inv", gauntlet_inv_formspec)
        end
    end
...
function inventorybags.register_bag(name, bagtable)
    minetest.register_tool(":" .. name, {
        ...
        on_use = function(itemstack, user, pointed_thing)
            return inventorybags.on_use(itemstack, user, pointed_thing)  --inventorybags.on_use_bag(itemstack, user, pointed_thing)
        end,
...
minetest.register_tool(":inventorybags:bag_transporting_bag", {
