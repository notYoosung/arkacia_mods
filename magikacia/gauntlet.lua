...
local gauntlet_use_primary = function(itemstack, placer, pointed_thing)
    ...
    if pos then
        if has_in_gauntlet(placer, modname .. ":rune_fire") then
            for _, k in ipairs(around_circle_3_pos_list) do
                safe_replace({ x = pos.x + k[1], y = pos.y, z = pos.z + k[2] }, "mcl_fire:fire", placer)
            end
        end
        if has_in_gauntlet(placer, modname .. ":rune_lightning") then
            mcl_lightning.strike(pos)
        end
        if has_in_gauntlet(placer, modname .. ":rune_ender") then
            if pointed_thing.type == "node" then
                placer:set_pos({ x = pos.x, y = pos.y - 0.5, z = pos.z })
            else
                placer:set_pos(pos)
            end
        end
        if has_in_gauntlet(placer, modname .. ":rune_earth") then
            local meta = placer:get_meta()
            meta:set_float("magikacia:spell_earth_time_active", (meta:get_float("magikacia:spell_earth_time_active") or 0) + 1)
            placer:add_velocity({x=0, y=100, z=0})
        end
        ...
        minetest.sound_play("mcl_enchanting_enchant", { pos = pos, max_hear_distance = 64 }, true)
    end
    return nil
end
...
if not mcl_util._magikacia_init_fields then
    ...
    mcl_player.register_globalstep(function(player, dtime)
        local pos = player:get_pos()
        local nunderpos = { pos.x, pos.y - 0.1, pos.z }
        local nodeunder = minetest.get_node(nunderpos)
        local meta = player:get_meta()
        local spell_earth_time = meta:get_int("magikacia:spell_earth_time_active")

        if spell_earth_time > 0 then
            if nodeunder.walkable == false then
                meta:set_float("magikacia:spell_earth_time_active", spell_earth_time + dtime)
                for k, v in minetest.objects_inside_radius(pos, 8) do
                    local obj = v
                    if obj:get_luaentity() ~= nil
                        and obj:get_luaentity().name ~= modname .. ":rw_forceblast"
                    ...
                    then
                        obj:add_velocity({ x = 10 * (-pos.x), y = 30 * (-1 / pos.y) + 5, z = 10 * (-pos.z) })
                    end
                end
            else
