...
local function reset_upgraded_item(pos)
    ...
    elseif template_present and is_armor and not is_trimmed and mcl_smithing_table.is_smithing_mineral(inv:get_stack("mineral", 1):get_name()) then
        upgraded_item = mcl_smithing_table.upgrade_trimmed(inv:get_stack("upgrade_item", 1), inv:get_stack("mineral", 1),
            inv:get_stack("template", 1))
    end
...
minetest.register_node("mcl_smithing_table:table", {
    ...
    "A smithing table is a utility block used to alter tools and armor at the cost of a smithing template and the appropriate material. This is the only way to obtain trimmed armor or upgrade diamond equipment with netherite. It also serves as a toolsmith's job site block."),
    _doc_items_usagehelp = S(
        "Rightclick on a smithing table to access its interface. Put armor or tools in the upper left slot. The top right slot is reserved for mineral items. The bottom slot is for smithing templates. To upgrade your diamond armor and tools to netherite, the netherite upgrade template is required.") ..
        "\n" ..
        S(
        "To trim your armor, you need a mineral item and a smithing template. Each piece of armor can be given an trimming pattern. The items are consumed after trimming and the armor piece receives the pattern defined by the template.") ..
        "\n\n" ..
        S(
        ...
        if listname == "upgraded_item" then
            ...
            elseif mcl_armor.is_trimmed(stack) then
                ...
                if not mcl_achievements.award_unlocked(playername, "mcl:lots_of_trimming") and achievement_trims[template_name] then
                    ...
                    local used_achievement_trims = minetest.deserialize(meta:get_string(
                    "mcl_smithing_table:achievement_trims")) or {}
                    ...
                    if used_all then
                        ...
                        meta:set_string("mcl_smithing_table:achievement_trims",
                            minetest.serialize(used_achievement_trims))
                    end
                end
            end
