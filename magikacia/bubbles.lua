if not vector.random_direction then
    function vector.random_direction()
        local x, y, z, l2
        repeat
            x, y, z = math.random() * 2 - 1, math.random() * 2 - 1, math.random() * 2 - 1
            l2 = x * x + y * y + z * z
        until l2 <= 1 and l2 >= 1e-6
        local l = math.sqrt(l2)
        return vector.new(x / l, y / l, z / l)
    end
end

magikacia_bubbles = {}

local blank = "blank.png^[png:"
magikacia_bubbles.textures = {
    bubble_block = "",
    bubble_block_pop = "",
    bubble_blower = "",
}
for k, v in pairs(magikacia_bubbles.textures) do
    magikacia_bubbles.textures[k] = blank .. v
end



function magikacia_bubbles.spawn_pop_particle(pos)
    local pdef = {
        amount = 1,
        pos = pos,
        velocity = { x = 0, y = 0, z = 0 },
        acceleration = { x = 0, y = 0, z = 0 },
        expirationtime = 0.25,
        size = 10,
        collisiondetection = false,
        collision_removal = false,
        object_collision = true,
        vertical = false,
        texture = {
            name = magikacia_bubbles.textures.bubble_block_pop,
        },
        animation = {
            type = "vertical_frames",
            aspect_w = 16,
            aspect_h = 16,
            length = 0.5,
        },
        glow = 14,
    }
    minetest.add_particle(pdef)
end


magikacia_bubbles.bubble_colors = {
    "#d3a4f7",
    "#f3c1f0",
    "#a2c0f8",
    "#a2f8df",
}



for i, v in ipairs(magikacia_bubbles.bubble_colors) do
    minetest.register_node(":magikacia:bubble_block_" .. i, {
        description = "Bubble Block\nFall Damage: 0%\nBreaks when u stand on it!",
        tiles = { magikacia_bubbles.textures.bubble_block .. "^[colorize:" .. magikacia_bubbles.bubble_colors[i] .. ":alpha" },
        groups = { dig_immediate = 3, fall_damage_add_percent = -100, },
        sounds = {
            dug = {name = "slimenodes_dug", gain = 0.5},
            place = {name = "slimenodes_place", gain = 0.5},
            footstep = {name = "slimenodes_step", gain = 0.3},
        },
        use_texture_alpha = "blend",
        paramtype = "light",
        is_ground_content = false,
        sunlight_propagates = true,
        on_destruct = function(pos)
            magikacia_bubbles.spawn_pop_particle(pos)
        end,
    })
end





function magikacia_bubbles.bubble_blower_primary(itemstack, user, pointed_thing)
    if user and user:get_pos() then
        local look_dir = user:get_look_dir()
        local pos = vector.offset(user:get_pos(), look_dir.x, (user:get_properties().eye_height or 1.625) + look_dir.y, look_dir.z)
        minetest.add_particlespawner({
            amount = 100,
            time = 1,
            minpos = pos,
            maxpos = pos,
            minvel = vector.offset(look_dir, -1, -1, -1),
            maxvel = vector.offset(look_dir, 1, 1, 1),
            minacc = vector.zero(),
            maxacc = vector.multiply(look_dir, 5),
            minexptime = 1,
            maxexptime = 3,
            minsize = 1,
            maxsize = 5,
            collisiondetection = true,
            collision_removal = true,
            object_collision = true,
            glow = 14,
            texture = magikacia_bubbles.textures.bubble_block_pop,
            animation = {
                type = "vertical_frames",
                aspect_w = 16,
                aspect_h = 16,
                length = 1,
            },
        })
        return itemstack, pos
    end
end
function magikacia_bubbles.bubble_blower_secondary(itemstack, user, pointed_thing)
    if pointed_thing then
        local pos
        if pointed_thing.type == "node" then
            pos = pointed_thing.above
        elseif pointed_thing.type == "object" then
            local obj = pointed_thing.ref
            if obj then
                pos = obj:get_pos()
            end
        else
            if user then
                local look_height = user:get_properties().eye_height or 1.625
                pos = vector.add(vector.offset(user:get_pos(), 0, look_height, 0), vector.multiply(user:get_look_dir(), 3))
            end
        end
        if pos then
            local node = minetest.get_node(pos)
            if node and (node.name == "air" or minetest.registered_nodes[node.name] and minetest.registered_nodes[node.name].buildable_to) then
                minetest.place_node(pos, { name = "magikacia:bubble_block_" .. math.ceil(math.random() * 4) }, user)
            end
            return itemstack, pos
        end
    end
end
minetest.register_tool(":magikacia:bubble_blower", {
    description = "Bubble blower",
    inventory_image = magikacia_bubbles.textures.bubble_blower .. mcl_enchanting.overlay,
    on_use = magikacia_bubbles.bubble_blower_primary,
    on_secondary_use = magikacia_bubbles.bubble_blower_secondary,
    on_place = magikacia_bubbles.bubble_blower_secondary,
})












if not mcl_util._magikacia_buuble_spawner_init then
    local bubbledestroyfunc = function(player)
        local pos = player:get_pos()
        local npos = vector.add(pos, mcl_player.node_offsets.stand)
        local node = minetest.get_node(npos)
        if node and node.name:find("magikacia:bubble_block_") then
            minetest.swap_node(npos, { name = "air" })
            minetest.sound_play({ name = "slimenodes_dug", gain = 0.5 }, { pos = npos })
            magikacia_bubbles.spawn_pop_particle(npos)
        end
    end
    table.insert(mcl_player.registered_globalsteps_slow, bubbledestroyfunc)
    mcl_util._magikacia_buuble_spawner_init = true
end

