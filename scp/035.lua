...
local function mask_dialog_say(player, msgtype)
    local msg = table.random_element(dialogs[msgtype])
    if msgtype == "ambient" or msgtype == "hostile" then
        msg = msg .. minetest.colorize(mcl_colors.RED, "")
    end
    mask_say(player, "[SCP-035] " .. msg)
end
...
local function drop_mask(player)
    ...
    if armor_list[2]:get_name() == "scp:scp_035" then
        ...
        if inv:contains_item("main", "scp:scp_035") then
