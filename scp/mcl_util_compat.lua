...
function table.update(t, ...)
	for _, to in ipairs {...} do for k, v in pairs(to) do t[k] = v end end
	return t
end

-- Updates nil values in t using values from to*.
function table.update_nil(t, ...)
	for _, to in ipairs {...} do for k, v in pairs(to) do if t[k] == nil then t[k] = v end end end
	return t
end
...
function table.count(t, does_it_count)
	local r = 0
	for k, v in pairs(t) do if does_it_count == nil or (type(does_it_count) == "function" and does_it_count(k, v)) then r = r + 1 end end
	return r
end

function table.random_element(t)
	local keyset = {}
	for k, _ in pairs(t) do table.insert(keyset, k) end
	local rk = keyset[math.random(#keyset)]
	return t[rk], rk
end

function mcl_util.get_luaentity_by_id(id) for _, e in pairs(minetest.luaentities) do if e._id == id then return e end end end
...
function mcl_util.rotate_axis_and_place(itemstack, placer, pointed_thing, infinitestacks, _)
	...
	if not ndef or not ndef.buildable_to then return end
...
function mcl_util.rotate_axis(itemstack, placer, pointed_thing)
	if placer and placer:is_player() then return mcl_util.rotate_axis_and_place(itemstack, placer, pointed_thing, minetest.is_creative_enabled(placer:get_player_name()), placer:get_player_control().sneak) end
...
function mcl_util.get_eligible_transfer_item_slot(src_inventory, src_list, dst_inventory, dst_list, condition)
	...
	for i = 1, size do
		...
		if not stack:is_empty() and (condition == nil or condition(stack, src_inventory, src_list, dst_inventory, dst_list)) then return i end
	end
	return nil
end
...
function mcl_util.move_item(source_inventory, source_list, source_stack_id, destination_inventory, destination_list)
	if source_stack_id == -1 then
		...
		if source_stack_id == nil then return false end
	end
	...
	if not source_inventory:is_empty(source_list) then
		...
		if not stack:is_empty() then
			...
			if not destination_inventory:room_for_item(destination_list, new_stack) then return false end
			stack:take_item()
...
function mcl_util.move_item_container(source_pos, destination_pos, source_list, source_stack_id, destination_list)
	...
	if sctype == 7 then return false end
	...
	local function normalize_double_container(pos, node, ctype)
		if ctype == 6 then
			...
			if ctype ~= 5 then return false end
		end
	...
	if not source_stack_id then source_stack_id = -1 end
	if source_stack_id == -1 then
		local cond = nil
		-- Prevent shulker box inception
		if dctype == 3 then cond = is_not_shulker_box end
		...
		if not source_stack_id then
			...
			if snode and sctype == 5 then
				...
				source_stack_id = mcl_util.get_eligible_transfer_item_slot(sinv, source_list, dinv, dpos, cond)
				if not source_stack_id then return false end
			else
				return false
			end
		end
	...
	if dctype == 3 then
		...
		if stack and minetest.get_item_group(stack:get_name(), "shulker_box") == 1 then return false end
	end
	-- Container type 7 does not allow any placement
	if dctype == 7 then return false end
	...
	return function(pos, _, oldmetadata)
		if oldmetadata and oldmetadata.inventory then
			for _, listname in pairs(lists) do
				...
				if list then for _, stack in pairs(list) do drop_item_stack(pos, stack) end end
			end
		else
			local meta = minetest.get_meta(pos)
			local inv = meta:get_inventory()
			for _, listname in pairs(lists) do for i = 1, inv:get_size(listname) do drop_item_stack(pos, inv:get_stack(listname, i)) end end
			meta:from_table()
		end
	end
end
...
function mcl_util.get_burntime(item)
	...
	if fuel_cache[item] == nil then
		fuel_cache[item] = minetest.get_craft_result({
			method = "fuel",
			width = 1,
			items = {item}
		}).time
...
-- Returns false otherwise
function mcl_util.is_fuel(item) return mcl_util.get_burntime(item) ~= 0 end
...
function mcl_util.generate_on_place_plant_function(condition)
	return function(itemstack, placer, pointed_thing)
		...
		if not def_under or not def_above then return itemstack end
		...
		if result == true then
			...
			if success then
				if idef.sounds and idef.sounds.place then
					minetest.sound_play(idef.sounds.place, {
...
function mcl_util.get_color(colorstr)
	...
	if hex then return colorstr, hex end
end
...
function mcl_util.call_on_rightclick(itemstack, player, pointed_thing)
	...
	if pointed_thing and pointed_thing.type == "node" then
		...
		if player and player:is_player() and not player:get_player_control().sneak then
			...
			if on_rightclick then return on_rightclick(pos, node, player, itemstack, pointed_thing) or itemstack end
		end
	end
end
...
function mcl_util.calculate_durability(itemstack)
	...
	if armor_uses > 0 then
		uses = armor_uses
		if unbreaking_level > 0 then uses = uses / (0.6 + 0.4 / (unbreaking_level + 1)) end
	else
		...
		if def then
			...
			if fixed_uses then
				uses = fixed_uses
				if unbreaking_level > 0 then uses = uses * (unbreaking_level + 1) end
			end
		end
...
function mcl_util.deal_damage(target, damage, mcl_reason)
	...
	if luaentity then
		damage = mcl_damage.run_modifiers(target, damage, mcl_reason or {
			type = "generic"
		})
		if luaentity.deal_damage then
			if luaentity:deal_damage(damage, mcl_reason or {
				type = "generic"
			}) ~= true then
				mcl_damage.run_damage_callbacks(target, damage, mcl_reason or {
					type = "generic"
				})
			end
			return
		elseif luaentity.is_mob then
			...
			if luaentity.health - damage > 0 then
				...
				luaentity.health = luaentity.health - damage
				mcl_damage.run_death_callbacks(target, mcl_reason or {
					type = "generic"
				})
			end
			...
			if armorgroups and not armorgroups.immortal then
				...
				if puncher and puncher.get_pos and puncher:get_pos() and target and target.get_pos and target:get_pos() and target.punch then
					...
					}, vector.direction(puncher:get_pos(), target:get_pos()), damage)
				end
			end
		end
	end
...
function mcl_util.get_inventory(object, create)
	if object:is_player() then
		...
		if create and not inventory and luaentity.create_inventory then inventory = luaentity:create_inventory() end
...
function mcl_util.get_object_name(object)
	if object:is_player() then
		...
		if not luaentity then return tostring(object) end
...
function mcl_util.replace_mob(obj, mob)
	...
	if not obj or not obj:get_pos() then return end
	if l.on_mob_replace then l:on_mob_replace(obj:get_luaentity()) end
	l:safe_remove()
	l = obj:get_luaentity()
	if l.is_mob then
		l:set_nametag(n)
...
function mcl_util.get_pointed_thing(player, objects, liquid, ignore)
	...
	if ignore then for pointed_thing in ray do if (type(ignore) == "table" and table.indexof(ignore, minetest.get_node(pointed_thing.under).name) == -1) or (type(ignore) == "function" and not ignore(pointed_thing)) then return pointed_thing end end end
	return ray:next()
end
...
function mcl_util.set_properties(obj, props)
	...
	if changed then obj:set_properties(p) end
end
	...
	if not pos_equal or not rot_equal then
		if obj.set_bone_override then -- when < minetest 5.9 isn't supported anymore remove these checks and only use the "override" variant and radians
			obj:set_bone_override(bone, {
				position = {
					vec = pos or current_pos,
					absolute = true
				},
				rotation = {
					vec = vector.apply(rot or current_rot, math.rad),
					absolute = true
				}
			})
		else
...
function mcl_util.bypass_buildable_to(func)
	...
	local function user_name(user) return user and user:get_player_name() or "" end

	local function log(name, ...) return name ~= "" and minetest.log(...) end
	...
	local function check_attached_node(p, n, group_rating)
		...
		if def2 and not def2.walkable then return false end
		return true
	end
	...
	return function(itemstack, placer, pointed_thing, param2)
		...
		-------------------
		local def = itemstack:get_definition()
		if def.type ~= "node" or pointed_thing.type ~= "node" then return itemstack end
		...
		if not oldnode_under or not oldnode_above then
			log("info", playername .. " tried to place" .. " node in unloaded position " .. minetest.pos_to_string(above))
			return itemstack
		end
		...
		if not olddef_above.buildable_to and not olddef_under.buildable_to then
			log("info", playername .. " tried to place" .. " node in invalid position " .. minetest.pos_to_string(above) .. ", replacing " .. oldnode_above.name)
			return itemstack
		end
		...
		if minetest.is_protected(place_to, playername) then
			log("action", playername .. " tried to place " .. def.name .. " at protected position " .. minetest.pos_to_string(place_to))
		...
		elseif (def.paramtype2 == "wallmounted" or def.paramtype2 == "colorwallmounted") and not param2 then
			local dir = vector.subtract(under, above)
			newnode.param2 = minetest.dir_to_wallmounted(dir)
			-- Calculate the direction for furnaces and chests and stuff
		elseif (def.paramtype2 == "facedir" or def.paramtype2 == "colorfacedir" or def.paramtype2 == "4dir" or def.paramtype2 == "color4dir") and not param2 then
		...
		if an ~= 0 and not check_attached_node(place_to, newnode, an) then
			log("action", "attached node " .. def.name .. " cannot be placed at " .. minetest.pos_to_string(place_to))
			return itemstack
		end

		log("action", playername .. " places node " .. def.name .. " at " .. minetest.pos_to_string(place_to))
		...
		if def.after_place_node then
			...
			if def.after_place_node(place_to_copy, placer, itemstack, pointed_thing_copy) then take_item = false end
		end
		...
		for _, callback in ipairs(minetest.registered_on_placenodes) do
			...
			local pointed_thing_copy = copy_pointed_thing(pointed_thing)
			if callback(place_to_copy, newnode_copy, placer, oldnode_copy, itemstack, pointed_thing_copy) then take_item = false end
		end
...
function mcl_util.safe_place(pos, node, player, itemstack)
	local name = player and player:get_player_name() or ""
	local nnode = node or (itemstack and {
...
function mcl_util.traverse_tower(pos, dir, callback)
	...
	while minetest.get_node(pos).name == node.name do
		if callback and callback(pos, dir, node) then return pos, i, true end
		i = i + 1
		...
		for y = pos1.y, pos2.y do
			for x = pos1.x, pos2.x do
				...
				if is_group then
					...
					if minetest.get_item_group(node_name, group_name) > 0 then data[vi] = c_to end
				else
...
mcl_util.bulk_swap_node = minetest.bulk_swap_node or function(positions, node) for _, pos in pairs(positions) do minetest.swap_node(pos, node) end end
...
function mcl_util.circle_bulk_set_node_vm(radius, pos, y, mat_to, param2)
	...
	if param2 then param2data = vm:get_param2_data() end
...
function mcl_util.create_ground_turnip(pos, fwidth, fdepth)
	...
	if reg_biome and reg_biome.node_top then
		...
		if reg_biome.node_filler then
			...
			if minetest.get_item_group(filler, "material_sand") > 0 then if reg_biome.node_stone then filler = reg_biome.node_stone end end
...
local function get_visual_size(obj)
	return obj:is_player() and {
		x = 1,
		y = 1,
		z = 1
	} or obj:get_luaentity()._old_visual_size or obj:get_properties().visual_size
end
...
function mcl_util.detach_object(obj, change_pos, callback)
	...
	obj:set_detach()
	obj:set_properties({
		visual_size = get_visual_size(obj)
	})
	...
	if change_pos then obj:set_pos(vector.add(obj:get_pos(), change_pos)) end
	if callback then
...
local roman_conversion = {{1000, "M"}, {900, "CM"}, {500, "D"}, {400, "CD"}, {100, "C"}, {90, "XC"}, {50, "L"}, {40, "XL"}, {10, "X"}, {9, "IX"}, {5, "V"}, {4, "IV"}, {1, "I"}}
