minetest.register_craftitem(":scp:scp_018_spawner", {
    ...
    inventory_image = "mcl_colorblocks_concrete_red.png",
    stack_max = 1,
    ...
    for _, v in ipairs(objs) do
        ...
        if le then
            if le.name == "scp:scp_018" then
                v:remove()
                minetest.chat_send_player(placer:get_player_name(), minetest.colorize("#f00", "[SCP Remover] Eliminated instance of SCP-018."))
            end
        end
    end
end
...
minetest.register_entity(":scp:scp_018", {
    initial_properties = {
        ...
        stepheight = 1,
        visual = "wielditem",
        wield_item = "mcl_colorblocks:concrete_red",
        static_save = true,
    },
    ...
    on_step = function(self, dtime, moveresult)
        ...
        if moveresult.collides and moveresult.collisions then
            ...
            for _, coll in pairs(moveresult.collisions) do
                change_vec[coll.axis] = -1
            end
            if moveresult.collisions[1] then
               velocity = vector.multiply(moveresult.collisions[1].old_velocity, change_vec)
            end
        end
        ...
        if speed > max_speed then
            self.object:set_velocity(vector.multiply(vector.normalize(self.object:get_velocity()), max_speed))
        end
