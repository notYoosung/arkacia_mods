local S = minetest.get_translator("mobs_mc")

local teams = {
	ci = {
		
	},
	foundation = {

	},
	scp_hostile = {

	},
	scp_safe = {

	},
}

local function register_player_mob(def)
	local mob_def = {
		description = def.description,
		type = "monster",
		spawn_class = "hostile",
		pathfinding = 1,
		hp_min = 20,
		hp_max = 20,
		xp_min = 6,
		xp_max = 6,
		collisionbox = { -0.3, -0.01, -0.3, 0.3, 1.90, 0.3 },
		visual = "mesh",
		mesh = "mcl_armor_character.b3d",
		head_swivel = "head.control",
		bone_eye_height = 2.2,
		head_eye_height = 2.2,
		curiosity = 100,
		textures = { "blank.png^[png:" .. def.texture, "blank.png", "blank.png" },
		visual_size = {
			x = 1,
			y = 1
		},
		makes_footstep_sound = true,
		damage = 13,
		reach = 2,
		walk_velocity = 1.2,
		run_velocity = 1.6,
		attack_type = "dogfight",
		specific_attack = { "player", },
		group_attack = true,
		attack_npcs = true,
		drops = { {
			name = "mcl_core:emerald",
			chance = 0,
			min = 0,
			max = 0,
			looting = "common"
		}, },
		animation = {
			stand_start = 40,
			stand_end = 49,
			stand_speed = 2,
			walk_start = 0,
			walk_end = 39,
			speed_normal = 25,
			run_start = 0,
			run_end = 39,
			speed_run = 50,
			punch_start = 50,
			punch_end = 59,
			punch_speed = 20,
		},
		view_range = 16,
		fear_height = 4
	}
	mcl_mobs.register_mob(":scp:player_mob_" .. def.name, mob_def)

	mcl_mobs.register_egg(":scp:player_mob_" .. def.name, def.description, "#333333", "#6F130B", 0)

end

register_player_mob({
	name = "mtf_epsilon_11",
	description = minetest.colorize(mcl_colors.RED, "MTF Epsilon-11 | \"NINE TAILED FOX\""),
	team = teams.foundation,
	texture = "",
})

register_player_mob({
	name = "class_d",
	description = minetest.colorize(mcl_colors.DARK_GREEN, "Class-D | Female"),
	team = teams.foundation,
	texture = "(mcl_skins_base_1_mask.png^[colorize:#EEB592FF:255)^mcl_skins_base_1.png^mcl_skins_footwear_3.png^mcl_skins_eye_1.png^(mcl_skins_bottom_1_mask.png^[colorize:#D0672AFF:alpha)^mcl_skins_bottom_1.png^(mcl_skins_top_5_mask.png^[colorize:#D0672AFF:alpha)^mcl_skins_top_5.png^(mcl_skins_hair_1_mask.png^[colorize:#715D57FF:alpha)^mcl_skins_hair_1.png",
})


minetest.register_alias("scp:playermodel_entity", "scp:player_mob_mtf_epsilon_11")