...
local function get_hardness_values_for_groups()
	...
	for g, _ in pairs(mcl_autogroup.registered_diggroups) do table.sort(values[g]) end
	return values
end
...
local function get_hardness_lookup_for_groups(hardness_values)
	...
	for g, values in pairs(hardness_values) do
		map[g] = {}
		for k, v in pairs(values) do map[g][v] = k end
	end
	return map
end
...
local function add_groupcaps(toolname, groupcaps, groupcaps_def, efficiency)
	if not groupcaps_def then return end
	...
	for g, capsdef in pairs(groupcaps_def) do
		...
		if def.levels then
			...
			if level > 0 then groupcaps[g .. "_dig_" .. def.levels[level]] = get_groupcap(g, true, mult, efficiency, uses) end
		else
...
function mcl_autogroup.can_harvest(nodename, toolname, player)
	...
	if minetest.get_item_group(nodename, "dig_immediate") >= 2 then return true end

	-- Check if it can be dug by tool
	local tdef = minetest.registered_tools[toolname]
	if tdef and tdef._mcl_diggroups then for g, gdef in pairs(tdef._mcl_diggroups) do if ndef.groups[g] then if ndef.groups[g] <= gdef.level then return true end end end end
...
local function get_tool_capabilities(tdef)
	if tdef.tool_capabilities then return tdef.tool_capabilities end
...
local function overwrite()
	...
	for nname, ndef in pairs(minetest.registered_nodes) do
		...
		if (nname ~= "ignore" and ndef.diggable) then
			...
			if (not (ndef.paramtype == "light" or ndef.sunlight_propagates)) and (ndef.groups.not_opaque == 0 or ndef.groups.not_opaque == nil) then newgroups.opaque = 1 end
			...
			for g, gdef in pairs(mcl_autogroup.registered_diggroups) do
				...
				if newgroups[g] then
					if gdef.levels then
						...
						for i = newgroups[g], #gdef.levels do newgroups[g .. "_dig_" .. gdef.levels[i]] = index end
					else
