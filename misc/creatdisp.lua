...
local dispenser_formspec = table.concat({
    ...
    "label[4.125,0.375;" .. F(C(mcl_formspec.label_color, S("Creative Dispenser"))) .. "]",
...
local function orientate(pos, placer, basename)
    ...
    if pitch_deg > 55 then
        minetest.swap_node(pos, { name = "arkacia:" .. basename .. "_up", param2 = node.param2 })
    elseif pitch_deg < -55 then
        minetest.swap_node(pos, { name = "arkacia:" .. basename .. "_down", param2 = node.param2 })
    end
end
...
local function activate_dispenser(pos, droppos, dropdir, inv, stack_)
    ...
    elseif dropnodedef and (not dropnodedef.walkable or stackdef._dispense_into_walkable) then
        if stackdef._on_dispense then -- Item-specific dispension (if defined)
            ...
            if od_ret then
                local newcount = stack:get_count()
                inv:set_stack("main", stack_id, stack)
                if newcount == 0 then
                    inv:set_stack("main", stack_id, od_ret)
...
local commdef = {
    ...
    groups = { pickaxey = 1, container = 2, material_stone = 1, dig_immediate = 3 },
...
do
    ...
    minetest.register_node(":arkacia:creative_dispenser", table.merge(dispenserdef, {
        description = S("Creative Dispenser"),
        ...
        after_place_node = function(pos, placer, itemstack, pointed_thing)
            setup_dispenser(pos)
            orientate(pos, placer, "creative_dispenser")
        end,
    }))

    minetest.register_node(":arkacia:creative_dispenser_down", table.merge(dispenserdef, {
        description = S("Creative Dispenser"),
        ...
        drop = "arkacia:creative_dispenser",
    }))
    minetest.register_node(":arkacia:creative_dispenser_up", table.merge(dispenserdef, {
        description = S("Creative Dispenser"),
        after_place_node = setup_dispenser,
        tiles = {
    ...
    minetest.register_craft({
        output = "arkacia:creative_dispenser",
        recipe = {
    ...
    doc.add_entry_alias("nodes", "arkacia:creative_dispenser", "nodes", "arkacia:creative_dispenser_down")
    doc.add_entry_alias("nodes", "arkacia:creative_dispenser", "nodes", "arkacia:creative_dispenser_up")
end
...
do
    ...
    minetest.register_node(":arkacia:creative_dropper", table.merge(dropperdef, {
        description = S("Creative Dropper"),
        ...
        _doc_items_usagehelp = S(
        "Creative Droppers can be placed in 6 possible directions, items will be dropped out of the hole. Use the dropper to access its inventory. Supply it with redstone energy once to make the dropper drop or transfer a random item."),
        tiles = {
            "default_furnace_top.png", "default_furnace_bottom.png",
            "default_furnace_side.png", "default_furnace_side.png",
        ...
        after_place_node = function(pos, placer, itemstack, pointed_thing)
            setup_dispenser(pos)
            orientate(pos, placer, "creative_dropper")
        end,
    }))

    minetest.register_node(":arkacia:creative_dropper_down", table.merge(dropperdef, {
        description = S("Creative Dropper"),
        after_place_node = setup_dispenser,
        tiles = {
        ...
        _doc_items_create_entry = false,
        drop = "arkacia:creative_dropper",
    }))
    minetest.register_node(":arkacia:creative_dropper_up", table.merge(dropperdef, {
        description = S("Creative Dropper"),
        after_place_node = setup_dispenser,
        tiles = {
